...
Future<List<String>> getChaptersByTestId(String testId) async {
  final response = await http
      .get(Uri.parse('http://192.168.115.51:5000/test/tests/$testId/chapters'));
...
class SceneStudentHome extends StatelessWidget {
                                            ...
                                            child: Container(
                                              decoration: BoxDecoration(
                                                ...
                                                color: isPast
                                                    ? Colors.red
                                                    : Colors
                                                        .green, // Red for past, green for future
                                              ),
                                              width: 7,
                                              height: 7,
                                            ),
  ...
  Widget buildTestButton(BuildContext context, TestInfo test, double fem) {
    ...
    bool isPast = test.date != null &&
        test.date!.isBefore(DateTime(now.year, now.month, now.day));
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  onPressed: isToday
                      ? () async {
                          // Only enable button if today
                          try {
                            List<Question> questions =
                                await getQuestionsByTestId(test.id!);
                            Navigator.of(context).push(MaterialPageRoute(
                              builder: (_) => TestPage(
                                  questions: questions, testId: test.id!),
                            ));
                          } catch (e) {
              ...
              if (isPast) // Show history icon only if the date is in the past
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                      builder: (_) => ResultQuiz(
                          studentId: '65df009a796124616d1ecdce',
                          testId: test.id!),
                    ));
                  },
                ),
