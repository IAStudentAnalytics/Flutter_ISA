...
// Envoyer les données pour la prédiction
  ...
  Future<void> sendPredictRequest(Map<String, double?> preparedData) async {
    final url = Uri.parse('http://172.16.1.188:5000/predict');
    final headers = {"Content-Type": "application/json"};
    final body = json.encode(preparedData);
  ...
  @override
  Widget build(BuildContext context) {
    List<double> realData = scores?.values.toList() ??
        [0, 0, 0, 0, 0]; // Default values if scores are null
    List<double> predictedData = predictedScores?.values.toList() ??
        [0, 0, 0, 0, 0]; // If predictedScores is null, use default values.
