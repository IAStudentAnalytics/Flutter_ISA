...
class TestProvider extends ChangeNotifier {
  ...
  Future<List<Test>> fetchTests() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Tests fetched successfully. Status code: ${response.statusCode} , $jsonData');
        if (jsonData != null && jsonData is Iterable) {
          final List<Test> tests =
              jsonData.map((json) => Test.fromJson(json)).toList();
          setTest(tests);
          return tests;
        } else {
  ...
  Future<void> addTest(String title, String description, int duration,
      List<Map<String, dynamic>> questions, DateTime testDate) async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'questions': questions,
          'testDate':
              testDate.toIso8601String(), // Convert DateTime to ISO 8601 string
        }),
        headers: {'Content-Type': 'application/json'},
      );
  ...
  Future<void> removeTest(String testId) async {
    try {
      final response =
          await http.delete(Uri.parse('$baseUrl/test/deleteTest/$testId'));
      if (response.statusCode == 200) {
